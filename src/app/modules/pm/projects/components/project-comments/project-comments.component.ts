...
export class ProjectCommentsComponent implements OnInit {
	...
	get commentControl() { return this.projectCommentsForm.controls; }

	get replyCommentControl() { return this.projectCommentsReplyForm.controls; }
	...
	onSubmit() {
		...
		this.projectCommentsService.create(this.projectCommentsForm.value).subscribe(data => {
			this.toastr.success('Cette transaction est ajouté avec succés', this.translate.instant('comments.title'));
		...
		Swal.fire({
			...
			if (result.value) {
				this.projectCommentsService.delete(commentId)
					.subscribe(
						data => {
							this.toastr.success('Cette transaction est supprimé avec succés', this.translate.instant('projects.title'));
							this.getComment(this.projectId);
						});
			}
		});
	...
	saveComments(comment, index, value) {
		...
		this.projectCommentsService.update(comment).subscribe(data => {
			this.toastr.success('Cette transaction est modifier avec succés', this.translate.instant('tasks.title'));
			this.getComment(this.projectId);
		});
	}
