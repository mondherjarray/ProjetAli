...
export class DefectCreateComponent implements OnInit {
	...
	loadForms() {
		...
		this.createDefectForm = this.formBuilder.group({
			...
			project_version: [0, Validators.required],
	...
	get defectControl() { return this.createDefectForm.controls; }
	...
	onSubmit() {
		this.projectService.getProjectById(this.createDefectForm.value.project_id)
			.subscribe(
				data => {
					this.project = data;
					
					this.createDefectForm.value.assign_member = this.project.assign_members;
		            this.createDefectForm.value.assigned_group_id = this.project.assign_to;
					console.log(this.project.budget_recu);
				});
		
		this.isFormSubmitted = true;
		this.createDefectForm.value.status = 1;
		this.createDefectForm.value.severity = 2;
		this.createDefectForm.value.defect_type = 1;
