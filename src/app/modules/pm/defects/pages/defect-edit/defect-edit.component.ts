...
export class DefectEditComponent implements OnInit {
	...
	loadForms() {
		...
		this.editDefectForm = this.formBuilder.group({
			...
			project_version: [this.defect.project_version, Validators.required],
	...
	get defectControl() { return this.editDefectForm.controls; }
	...
	getProjects() {
		this.projectService.getMyProjects().subscribe(data => {
			...
			this.projectVersion = selectedProject.project_version.split(",").reverse();
		});
	}
